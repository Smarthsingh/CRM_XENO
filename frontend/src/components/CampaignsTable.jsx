...
export default function CampaignsTable() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setSendDialog({ open: false, campaignId: null, campaignName: '', message: '', suggestions: [], loadingAI: false });
      fetchCampaigns();
      fetchStats();
    } catch {
  ...
  const handleAISuggestions = async () => {
    setSendDialog(prev => ({ ...prev, loadingAI: true }));
    try {
      const res = await fetch(`${API_URL}/api/ai/suggest-message`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          objective: `Suggest 3 short, engaging marketing messages for the campaign "${sendDialog.campaignName}". 
          Make them creative and personalized for customers.`,
        }),
      });
      const data = await res.json();
      if (data.suggestions?.length) {
        setSendDialog(prev => ({ ...prev, suggestions: data.suggestions, loadingAI: false }));
      } else {
        setSnackbar({ open: true, message: "No AI suggestions received.", severity: "warning" });
        setSendDialog(prev => ({ ...prev, loadingAI: false }));
      }
    } catch (err) {
  ...
  return (
    <Box sx={{ width: '100%', py: 4, background: '#f6fafd', minHeight: '100vh' }}>
                    ...
                    <TableCell align="center">
                      <Stack direction="row" spacing={1} justifyContent="center">
                        ...
                        <Tooltip title="Send Message">
                          <IconButton color="success" onClick={() => setSendDialog({ open: true, campaignId: row._id, campaignName: row.name, message: '', suggestions: [], loadingAI: false })} sx={{ borderRadius: 2, background: '#e6f9f0', '&:hover': { background: '#c6f6e6' } }}><SendIcon /></IconButton>
                        </Tooltip>
                      </Stack>
                    </TableCell>
                  </TableRow>
      ...
      {/* Send Message Dialog */}
      <Dialog open={sendDialog.open} onClose={() => setSendDialog({ open: false, campaignId: null, campaignName: '', message: '', suggestions: [], loadingAI: false })} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ minWidth: 500 }}>
          ...
          <Button
            ...
            disabled={sendDialog.loadingAI}
            sx={{ mt: 2, mb: 2 }}
          >
            {sendDialog.loadingAI ? "Loading..." : "✨ Get AI Suggestions"}
          </Button>

          {/* Render Suggestions */}
          {sendDialog.suggestions?.length > 0 && (
            <List>
              {sendDialog.suggestions.map((s, idx) => (
                <ListItem 
                  key={idx} 
                  button 
                  onClick={() => setSendDialog(prev => ({ ...prev, message: s }))}
        ...
        <DialogActions>
          <Button onClick={() => setSendDialog({ open: false, campaignId: null, campaignName: '', message: '', suggestions: [], loadingAI: false })}>Cancel</Button>
